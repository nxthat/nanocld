...
/// For example if we create a cluster `prod` in the default namespace `global`
/// The cluster key will be `global-prod`
...
///
/// ## Example
/// Generate the key of network `front` inside the cluster `prod` in namespace `global`
/// ```rust,norun
/// let key = gen_key("global-prod", "front");
/// ```
